...
# -*- coding: utf-8 -*-
"""
PCF Processor v7.0 - PCF file processor with selective report generation
...
class PCFProcessor:
 ...
 # -- revision status and indexing helpers -------------------------
    ...
    def _enrich_lengths_with_status_and_index(
        ...
        # ✨ Step 1: Group by LINEID (outer grouping)
        lineid_groups: dict[str, list[dict]] = {}
        for record in table3:
            lineid = record.get('LINEID', 'UNKNOWN')
            if lineid not in lineid_groups:
                lineid_groups[lineid] = []
            lineid_groups[lineid].append(record)
        
        enriched: list[dict] = []
        total_lineid_groups = 0
        
        # ✨ Step 2: Process each LINEID separately
        for lineid, lineid_records in lineid_groups.items():
            
            # ✨ Step 3: Group by (Date, Revision, Folder_Path) within this LINEID
            sub_grouped: dict[tuple, list[dict]] = {}
            for record in lineid_records:
                date = record.get('Date', '')
                revision = record.get('Revision', '')
                folder_path = record.get('Folder Path', '')
                
                key = (date, revision, folder_path)
                
                if key not in sub_grouped:
                    sub_grouped[key] = []
                sub_grouped[key].append(record)
            
            # ✨ Step 4: Sort sub-groups by (Date, Revision)
            sorted_sub_groups = sorted(
                sub_grouped.items(),
                key=lambda item: (item[0][0], item[0][1])  # Sort by Date, Revision
            )
            
            # ✨ Step 5: Assign Index and Status within this LINEID
            total_sub_groups = len(sorted_sub_groups)
            
            for sub_idx, ((date, revision, folder_path), records) in enumerate(sorted_sub_groups, start=1):
                is_latest = (sub_idx == total_sub_groups)
                
                for record in records:
                    record['Index'] = sub_idx
                    record['Status'] = 'Actual' if is_latest else 'Obsolete'
                    enriched.append(record)
            
            total_lineid_groups += total_sub_groups
        
        self.logger.info(
            f"Enriched {len(enriched)} length records with Status and Index. "
            f"Processed {len(lineid_groups)} LINEIDs with {total_lineid_groups} total unique combinations."
        )
        return enriched
    ...
    def parse_pcf_for_lengths(
        ...
        # Global UNITS-BORE (first 20 lines)
    ...
    def _collect_data(
        ...
        for i, fp in enumerate(pcf_files):
            ...
            pipeline_data, instruments = self.parse_pcf_file(fp)
            ...
            if generate_reports.get('lengths'):
                for rec in self.parse_pcf_for_lengths(fp, include_continuation):
                    table3.append({
                        'Name': filename,
                        'LINEID': rec['LINEID'],
                        'SIZE': rec['SIZE'],
