...
class PCFProcessor:
    ...
    def parse_pcf_file(self, file_path):
        ...
        for block in instrument_blocks:
            ...
            # Extract CENTRE-POINT coordinates
            centre_match = re.search(r'CENTRE-POINT\s+([\d.-]+)\s+([\d.-]+)\s+([\d.-]+)', block_content)
            
            if centre_match:
                # Use CENTRE-POINT if found
                x = centre_match.group(1)
                y = centre_match.group(2)
                z = centre_match.group(3)
            else:
                # Otherwise use first END-POINT
                endpoint_match = re.search(r'END-POINT\s+([\d.-]+)\s+([\d.-]+)\s+([\d.-]+)', block_content)
                if endpoint_match:
                    x = endpoint_match.group(1)
                    y = endpoint_match.group(2)
                    z = endpoint_match.group(3)
                else:
                    # No coordinates found - skip or use empty
                    x, y, z = '', '', ''
