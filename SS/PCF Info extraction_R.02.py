...
class PCFProcessor:
    ...
    def process_folder(self, folder_path, progress_callback=None, save_paths=None):
        """
        ...
        save_paths: dict с ключами 'pipeline' и 'instruments' для путей сохранения файлов,
                    если None - используется дефолтная логика с временнАми метками
        """
        # Поиск PCF файлов
        pcf_files = []
        ...
        try:
            ...
            if save_paths:
                ...
                # Создаем имена файлов с временной меткой, если пути не переданы
            ...
            # Сохраняем таблицы с автошириной
...
def main():
    ...
    def select_and_process():
        folder_path = filedialog.askdirectory(title="Выберите папку с PCF файлами для обработки")
        if not folder_path:
            return
        
        # Запрашиваем папку и имя для сохранения PIPELINE файла
        pipeline_save_path = filedialog.asksaveasfilename(
            title="Сохранить файл с данными PIPELINE-REFERENCE как...",
            defaultextension=".xlsx",
            filetypes=[("Excel files", "*.xlsx")],
            initialfile="PCF_Pipeline_Attributes"
        )
        if not pipeline_save_path:
            messagebox.showinfo("Отмена", "Сохранение файла PIPELINE-REFERENCE отменено.")
            return
        
        # Запрашиваем папку и имя для сохранения INSTRUMENT файла
        instrument_save_path = filedialog.asksaveasfilename(
            title="Сохранить файл с данными INSTRUMENT как...",
            defaultextension=".xlsx",
            filetypes=[("Excel files", "*.xlsx")],
            initialfile="PCF_Instruments"
        )
        if not instrument_save_path:
            messagebox.showinfo("Отмена", "Сохранение файла INSTRUMENT отменено.")
            return
    ...
    select_button = tk.Button(
        root,
        text="Выбрать папку и начать обработку",
