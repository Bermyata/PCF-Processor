...
class PCFProcessor:
    ...
    def __init__(self):
        ...
        # Revision extraction settings
        self.revision_left_delimiter = ''
        self.revision_right_delimiter = ''
    ...
    def extract_date_from_path(self, file_path):
        ...
        Returns:
            str: Date string in format YYYY.MM.DD or empty string if not found
        """
        import re
        
        # Search for date pattern YYYY.MM.DD in path
    ...
    def extract_revision_from_filename(self, filename, left_delimiter='', right_delimiter=''):
        """
        Extract revision number from filename using user-defined delimiters
        
        Args:
            filename: Filename to extract revision from
            left_delimiter: Left delimiter (one or more characters)
            right_delimiter: Right delimiter (one or more characters)
            
        Returns:
            str: Revision number or empty string if not found
        """
        if not left_delimiter or not right_delimiter:
            return ''
        
        try:
            # Find last occurrence of left delimiter
            left_pos = filename.rfind(left_delimiter)
            if left_pos == -1:
                return ''
            
            # Start position after left delimiter
            ...
            # Extract revision between delimiters
            revision = filename[start_pos:right_pos].strip()
            
            return revision if revision else ''
            
        except Exception as e:
            self.logger.warning(f"Error extracting revision from {filename}: {e}")
            return ''    
    ...
    def add_error(self, pcf_name, pipeline_ref, error_type, full_path=None):
        ...
        revision = self.extract_revision_from_filename(pcf_name, 
                                                        self.revision_left_delimiter,
                                                        self.revision_right_delimiter) if self.revision_enabled and full_path else ''
    ...
    def extract_pipeline_refs_from_filename(self, filename):
        ...
        name = re.sub(r'[-_\s]*(?:rev|revision)\.?[-_\s]*\d+$', '', name, flags=re.IGNORECASE)
        return name.strip(' _-')
    ...
    def parse_pcf_file(self, file_path):
        """Parse PCF file for attributes and instruments with coordinates"""
        ...
        for line in lines:
            ...
            if in_pipeline_block:
                ...
                if any(stripped.startswith(block) for block in component_blocks):
                    break
        ...
        if 'PIPELINE-REFERENCE' not in pipeline_data:
            return None, []

        instrument_pattern = r'^INSTRUMENT\b[\s\S]*?(?=^(?:[A-Z][A-Z-]*)(?:\s|$)|\Z)'
        for block in re.finditer(instrument_pattern, content, re.MULTILINE):
            block_content = block.group(0)

            tag_match = re.search(r'^\s*TAG\s+(.+)$', block_content, re.MULTILINE)
            if not tag_match:
                continue
            ...
            centre_match = re.search(r'^\s*CENTRE-POINT\s+([\d.-]+)\s+([\d.-]+)\s+([\d.-]+)', block_content, re.MULTILINE)
            if centre_match:
                coords = centre_match.groups()
            else:
                endpoint_match = re.search(r'^\s*END-POINT\s+([\d.-]+)\s+([\d.-]+)\s+([\d.-]+)', block_content, re.MULTILINE)
                coords = endpoint_match.groups() if endpoint_match else ('', '', '')
    ...
    def process_folder(self, folder_path, progress_callback=None, save_paths=None, include_continuation=False, 
                        generate_reports=None, revision_settings=None):
        """
        ...
        Args:
            ...
            revision_settings: dict with 'enabled', 'left_delimiter', 'right_delimiter'
        """
        ...
        if revision_settings:
            self.revision_enabled = revision_settings.get('enabled', False)
            self.revision_left_delimiter = revision_settings.get('left_delimiter', '')
            self.revision_right_delimiter = revision_settings.get('right_delimiter', '')
            ...
            if pipeline_data:
                if generate_reports['pipeline']:
                    row_data = {
                        ...
                        'Revision': self.extract_revision_from_filename(filename, 
                                                         self.revision_left_delimiter, 
                                                         self.revision_right_delimiter) if self.revision_enabled else ''
                    }
        ...
        try:
            ...
            if generate_reports.get('pipeline'):
                ...
                if output_path1:
                    ...
                    self.logger.warning("Save path for Pipeline Attributes report not provided.")

            if generate_reports.get('instruments'):
                ...
                if output_path2:
                    ...
                    self.logger.warning("Save path for Instruments report not provided.")

            if generate_reports.get('lengths'):
                ...
                if output_path3:
                    ...
                    self.logger.warning("Save path for PipeLengthFromPCF report not provided.")

            if generate_reports.get('errors'):
                ...
                if output_path_errors:
                    ...
                    self.logger.warning("Save path for Errors report not provided.")
            ...
            files_created_text = '\n'.join(f"• {fname}" for fname in created_files) if created_files else "• No files created"
...
class ProgressWindow:
    """Progress window for displaying processing status"""
...
class RevisionDefinitionDialog:
    ...
    def __init__(self, parent, folder_path):
        ...
        self.window.geometry("700x550")  # Увеличена высота с 450 до 550
        self.window.resizable(False, False)
        self.window.transient(parent)
        self.window.grab_set()
        ...
        y = (self.window.winfo_screenheight() // 2) - (550 // 2)  # Обновлена позиция
        ...
        instruction_label = tk.Label(self.window, text=instruction_text, 
        ...
        select_btn = tk.Button(file_frame, text="Browse...", command=self.select_sample_file, padx=10)
        ...
        self.left_entry.bind('<KeyRelease>', self.update_preview)
        self.right_entry.bind('<KeyRelease>', self.update_preview)
        
        # Buttons - фиксированная позиция внизу
        button_frame = tk.Frame(self.window)
        button_frame.pack(side=tk.BOTTOM, pady=15)
        
        apply_btn = tk.Button(button_frame, text="Apply", command=self.apply, 
                             bg="#4CAF50", fg="white", padx=30, pady=8,
                             font=("Arial", 10, "bold"))
        apply_btn.pack(side=tk.LEFT, padx=10)
        
        cancel_btn = tk.Button(button_frame, text="Cancel", command=self.cancel, 
                              padx=30, pady=8, font=("Arial", 10))
        cancel_btn.pack(side=tk.LEFT, padx=10)
        
        self.sample_filename = None
    
    def select_sample_file(self):
        """Select sample PCF file"""
        file_path = filedialog.askopenfilename(
            title="Select sample PCF file",
            initialdir=self.folder_path,
            filetypes=[("PCF files", "*.pcf"), ("PCF files", "*.PCF"), ("All files", "*.*")]
        )
        
        if file_path:
            self.sample_filename = os.path.basename(file_path)
            self.file_label.config(text=self.sample_filename, fg="black")
            self.update_preview()
    
    def update_preview(self, event=None):
        """Update preview with current delimiters"""
        ...
        if not left or not right:
            self.preview_label.config(
                text=f"Filename: {self.sample_filename}\n\nEnter both delimiters to see result",
                fg="gray"
            )
            return
        
        # Extract revision using current delimiters
        try:
            left_pos = self.sample_filename.rfind(left)
            if left_pos == -1:
                self.preview_label.config(
                    text=f"Filename: {self.sample_filename}\n\n"
                         f"❌ Left delimiter '{left}' not found",
                    fg="red"
                )
                return
            
            start_pos = left_pos + len(left)
            right_pos = self.sample_filename.find(right, start_pos)
            
            if right_pos == -1:
                self.preview_label.config(
                    text=f"Filename: {self.sample_filename}\n\n"
                         f"❌ Right delimiter '{right}' not found after left delimiter",
                    fg="red"
                )
                return
            
            revision = self.sample_filename[start_pos:right_pos].strip()
            
            if not revision:
                self.preview_label.config(
                    text=f"Filename: {self.sample_filename}\n\n"
                         f"⚠ Nothing found between delimiters",
                    fg="orange"
                )
            else:
                self.preview_label.config(
...
def main():
    ...
    # Revision definition section  
    revision_frame = tk.LabelFrame(root, text="Step 3: Define Revision (Optional)", 
                                font=("Arial", 10, "bold"), padx=10, pady=10)
    revision_frame.pack(padx=20, pady=10, fill=tk.X)

    revision_status_label = tk.Label(revision_frame, text="Revision extraction: Disabled", 
                                    fg="gray", font=("Arial", 9))
    revision_status_label.pack(side=tk.LEFT, padx=10)

    revision_settings = {'enabled': False, 'left_delimiter': '', 'right_delimiter': ''}
    ...
    def open_results_folder(path):
        try:
            ...
            messagebox.showwarning("Warning", f"Unable to open folder:\n{exc}")

    def define_revision():
        """Open revision definition dialog"""
        if not selected_folder:
            messagebox.showwarning("Warning", "Please select a PCF folder first!")
            return
        
        dialog = RevisionDefinitionDialog(root, selected_folder)
        root.wait_window(dialog.window)
        
        if dialog.result:
            revision_settings.update(dialog.result)
            revision_status_label.config(
                text=f"Revision extraction: Enabled (Left: '{revision_settings['left_delimiter']}' "
                    f"Right: '{revision_settings['right_delimiter']}')",
                fg="green"
            )
        else:
            revision_settings['enabled'] = False
            revision_status_label.config(text="Revision extraction: Disabled", fg="gray")

    define_rev_btn = tk.Button(revision_frame, text="Define Revision Pattern", 
                            command=define_revision, padx=10, pady=5)
    define_rev_btn.pack(side=tk.RIGHT, padx=5)

    # Options section
    options_frame = tk.LabelFrame(root, text="Step 4: Options", font=("Arial", 10, "bold"), padx=10, pady=10)
    ...
    def start_processing():
        ...
        try:
            ...
            success = processor.process_folder(
                ...
                revision_settings=revision_settings  # Add this parameter
            )
            ...
            if success:
                if messagebox.askyesno("Open folder?", "Do you want to open results folder?"):
                    ...
                    folder_for_open = os.path.dirname(first_path) if first_path else ''
                    if folder_for_open:
                        open_results_folder(folder_for_open)
                    else:
                        processor.logger.warning("Unable to determine folder to open.")
